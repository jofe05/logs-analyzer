...
class RowDB:
    ...
    def add_row_to_db(self, c):
        ...
        c.execute("INSERT INTO partitions(jobId, architecture, mapper, threads, instances, counter, value, policy, dataset) VALUES (?,?,?,?,?,?,?,?,?)", listRow)    
        print("Row Added")
...
if __name__ == "__main__":
    ...
    currentRow = RowDB()
    foldersDataset = os.listdir(pathLogs + '/partitioning')
    try:
    ...
    if "Y" == input('Want to continue? [Y/N]'):        
        currentRow.expName = "partitioning"
        for dataset in foldersDataset: 
            logs = os.listdir(pathLogs + '/partitioning/' + str(dataset))
            ...
            for file in logs:
                ...
                iFile = open(pathLogs + '/partitioning/' + str(dataset) + '/' + str(file), 'r')
